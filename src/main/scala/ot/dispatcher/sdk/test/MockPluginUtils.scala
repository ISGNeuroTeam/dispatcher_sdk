...
class MockPluginUtils(sparkSession: SparkSession) extends PluginUtils{
  ...
  override def pluginConfig: Config = ConfigFactory.parseURL(Paths.get("src/test/resources/plugin.conf").toAbsolutePath.toUri.toURL)

  override def mainConfig: Config = ConfigFactory.parseURL(Paths.get("src/test/resources/application.conf").toAbsolutePath.toUri.toURL)
}
